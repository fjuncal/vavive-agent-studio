...
export function Field({ label, placeholder, textarea = false }: { label: string; placeholder?: string; textarea?: boolean }) {
  ...
  return (
    <label className="grid gap-1.5">
      ...
      {textarea ? <textarea className={className} rows={4} placeholder={placeholder} /> : <input className={className} placeholder={placeholder} />}
    </label>
  );
}
